...
class RefinedSynchronizer[T: ClassTag](val partitionID: Int,
      ...
      override def run(): Unit = {
        ...
        while (!shutdown) {
          try {
            ...
            if (blockFetchRequestQueue.size() > 5) {
              println(s"${new Date().toString} " +
                s"block fetching request queue size > 5 ${blockFetchRequestQueue.size()}")
            }
          //  println(s"fetching for ${fetchRequest.syncMeta.name}," +
         //     s"priority is ${fetchRequest.priority}")
    ...
    override def call(): Int = {
      ...
      if (aggregated) {
        ...
        if (syncMeta.stateOfWorld.size == totalPartition) {
          ...
          // TODO: Change above line back
        //  val updated = syncRequest.state.compareAndSet(1, 3 + syncMeta.partitionToCount)
          if (updated) {
            asyncTaskWaitingQueue.add(syncRequest)
          }
        }
  ...
  (0 until workerPoolSize).foreach(_ => {
    workerPool.submit(new Runnable {
      override def run(): Unit = {
        while (!shutdown) {
      //    println(s"current queuedepth ${asyncTaskWaitingQueue.size()}")
          val asyncTaskReq = asyncTaskWaitingQueue.take
          if (asyncTaskWaitingQueue.size() > 5) {
            println(s"~~~syn request queue depth > 5 ${asyncTaskWaitingQueue.size()}")
          }
