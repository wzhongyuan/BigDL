...
import com.intel.analytics.bigdl.dataset.image.{LabeledBGRImage, PreFetch}
...
object MTLabeledImageFeatureToBatch {
  ...
  : MTLabeledImageFeatureToBatch = {
    new MTLabeledImageFeatureToBatch (
      width, height, batchSize, transformer, toRGB)
  }
}

class MTLabeledImageFeatureToBatch private[bigdl](width: Int, height: Int,
      ...
      override def next(): MiniBatch[Float] = {
        ...
        val batch = Engine.default.invokeAndWait((0 until parallelism).map(tid => () => {
          ...
          while (iterators(tid).hasNext && {
           ...
           // println(img(ImageFeature.imageTensor))
